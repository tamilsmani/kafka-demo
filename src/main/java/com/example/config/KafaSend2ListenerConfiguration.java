...
@Component
@EnableKafka
public class KafaSend2ListenerConfiguration  {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(KafaSend2ListenerConfiguration.class);
	...
	public String listen(@Payload String payload,
		...
		LOGGER.info("Received message forwarded to sendTo topic[{}]",sendToTopic);
				
		return "sendTo" + payload;
		//ack.acknowledge();
	}
	
	@KafkaListener(id="${kafka.topic}-sendto", topics = "${kafka.topic}-sendto",	
	...
	public void sendTolisten(@Payload String payload,
		...
		//LOGGER.info("Message-without-groupid consumed from group[{}] from topic [{}] and payload[{}]", KafkaUtils.getConsumerGroupId(), topic, payload);
		LOGGER.info("Inside sendTo  message info offset[{}], Parition[{}], key[{}], timestamp[{}] and Ack[{}]", offset, partition);
		//ack.acknowledge();
	}
...
//			configMap.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
	...
	public NewTopic sendToTopic() {
	    return TopicBuilder.name(sendToTopic)
	            .partitions(3)
	            .replicas(3)
	            .compact()
	            .build();
